...
function App() {
  const [subjects, setSubjects] = useState([{ credit0: "", gradePoint0: "" }]);
  ...
  function computeCgpa() {
    const computedSubjects = subjects.map(function (subject, idx) {
      ...
      const creditPoints =
        subject[`credit${idx}`] * subject[`gradePoint${idx}`];
    ...
    let totalCredits = computedSubjects
      .map((subject, idx) => subject[`credit${idx}`])
      .reduce((prev, next) => Number(prev) + Number(next));
    let totalCreditPoints = computedSubjects
      .map((subject, idx) => subject[`creditPoints${idx}`])
      .reduce((prev, next) => Number(prev) + Number(next));
  ...
  return (
    <div className="App">
      ...
      <form onSubmit={(e) => handleSubmit(e)}>
        {subjects.map((subject, idx) => (
          <div key={idx}>
            ...
            <button
              ...
              disabled={idx + 1 === count ? false : true}
        ...
        <button className="button-cgpa" type="submit">
          Compute CGPA
        </button>
      </form>
